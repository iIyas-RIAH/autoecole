...
{
    $idseance = $_GET['modifier_idseance'];
    $Matricule = $_POST['matricule'];
    $CINMoniteur = $_POST['CIN'];
    $typeF = $_POST['typeF'];
    $jour = $_POST['jour'];
    $horaire = $_POST['horaire'];
    if ($Matricule=='NULL' || $typeF=='Théorique') {
    	$query_run = mysqli_query($connexion, "UPDATE Seance SET MATRICULE=NULL, CINMoniteur='$CINMoniteur', DATESEANCE='$jour', HORAIRE='$horaire', 	TYPEFORMATION ='$typeF' WHERE IDSEANCE='$idseance'");
    	header('Location: listSean.php');
    }else{
    	$query_run = mysqli_query($connexion, "UPDATE Seance SET MATRICULE='$Matricule', CINMoniteur='$CINMoniteur', DATESEANCE='$jour', HORAIRE='$horaire', 	TYPEFORMATION ='$typeF' WHERE IDSEANCE='$idseance'");
    	header('Location: listSean.php');  
	}
}
?>
...
<head>
  ...
  <title>Admin | Modifier une séance</title>
        ...
        <div class="container-fluid" style="padding-top: 25px;">
          ...
          <div class="card shadow mb-4">
            <div class="card-header py-3">
              <h5 class="m-0 font-weight-bold text-primary">Modifier une Séance</h5>
            </div>
            <div class="card-body">
      ...
      <div class="form">
        <form class="form__inner" method="post" accept-charset="utf-8">
          ...
          <div class="form__line"  style="margin-top: 15px;">
            ...
            <?php 
                $resultat = mysqli_query($connexion, "SELECT distinct dateseance FROM Seance");                 
            ...
            <?php 
                $resultat = mysqli_query($connexion, "SELECT distinct typeformation FROM Seance");                 
                while ($ligne4 = mysqli_fetch_row($resultat)) {
            ?>
                <option value="<?php echo $ligne4[0]; ?>"><?php echo $ligne4[0]; ?></option>
            <?php 
                }
            ?>
              </select>
